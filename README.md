...
| **Organisations** | Create org (pending super-admin approval), invite co-organizers, org settings |
...
| **Events** | Create/edit/delete events, public registration form, Indian state+city dropdowns, event gate password |
| **Registration Approval** | Registrations start as `not_approved`; admin/co-organizer must approve before the attendee sees their QR pass |
| **QR Passes** | Per-attendee QR code shown on My Events after approval; downloadable as PNG; mobile-optimised scanner with gate password |
| **3-stage Check-in** | `not_approved` → `admin_approved` → `approved (checked in)`. Approval ≠ check-in — attendees scan in at the door separately |
| **Dashboard** | Stats, event management, per-event team management, registrations table with bulk approve / bulk delete, CSV export |
| **Delete Capabilities** | Admin: delete registrations (single + bulk) and events · Super Admin: delete entire organisations (cascade) |
| **Super Admin** | Approve/reject/suspend orgs, delete orgs, full registrant drill-down per event, org detail view |
...
│   │       │   ├── registrations.ts  # Registrations + approve + bulk-approve + bulk-delete
...
│   │       │   └── super-admin.ts # SA auth, org management, org delete
│   │       ├── middleware/
...
│           ├── my-events/         # Attendee QR pass view (pending / approved+download / checked-in)
...
│           │   ├── page.tsx           # Stats, events list (with delete), create-org CTA
│           │   ├── events/[id]/       # Event detail + registrations (approve, bulk approve, delete, bulk delete)
│           │   │   ├── scan/          # Mobile QR scanner
│           │   │   └── team/          # Per-event team management
│           │   ├── team/              # Org-level team management
│           │   └── settings/          # Org settings
│           └── super-admin/       # SA panel (org approve/reject/suspend/delete)
├── packages/
...
| Role | Scope | Can do |
|------|-------|--------|
| `super_admin` | Platform | Approve/reject/suspend/delete orgs, view all data |
| `admin` | Org | Full org management, create/delete events, manage team, approve/delete registrations |
| `co_organizer` | Org or Event | View events, approve/delete registrations for assigned events, run scanner |
...
| Role | Can delete |
|------|-----------|
| `co_organizer` | Registrations for events they're assigned to |
| `admin` | Registrations (single + bulk) for any event in their org · Events in their org |
| `super_admin` | Everything above + entire organisations (cascades all events, registrations, members) |
...
```
not_approved  ──(admin approves)──►  admin_approved  ──(door scan)──►  approved
   (pending)                           (QR visible,                    (checked in)
                                        downloadable)
```

- Registrants see a **"Pending Approval"** state on My Events until approved.
- Admin/co-organizer approves from the dashboard registrations table (individually or bulk).
- On approval the attendee's QR pass becomes visible and downloadable as PNG.
- Physical entry is recorded separately when the gate scanner scans the QR — this sets `approved` (checked in).
...
| `/my-events` | Attendees | View registrations + QR passes (pending / approved+download / checked-in) |
...
| `/dashboard/events/:id` | Org members + co-organizers | Event detail, registrations, approve, bulk approve, delete |
...
5. **Attendee** opens `/e/:slug` → fills in the form → registration is `not_approved`
6. **Admin** goes to `/dashboard/events/:id` → approves the registration (individually or bulk)
7. **Attendee** opens `/my-events` → sees their QR pass and can download it as PNG
8. **Scanner** opens `/dashboard/events/:id/scan` on mobile → enters gate password → scans QR → entry approved (`checked in`)
9. Attendee's `/my-events` shows ✓ Checked in
...
GET    /registrations/event/:id        List registrations for event [admin/co_org]
POST   /registrations/bulk-approve     Approve multiple registrations [admin/co_org]
POST   /registrations/bulk-delete      Delete multiple registrations [admin/co_org]
PATCH  /registrations/:id/approve      Approve a single registration [admin/co_org]
DELETE /registrations/:id              Delete a single registration [admin/co_org]
POST   /registrations/:eventSlug       Register for event (authenticated users)

GET    /checkin/:uniqueCode            Look up registration by QR code
POST   /checkin/verify-password        Validate gate password before opening scanner
POST   /checkin/:uniqueCode            Record entry at door (requires gate password)
...
DELETE /super-admin/orgs/:id           Delete org + all cascade data
```
...
users (Supabase Auth)
  └── org_members  ─────────────── (role: admin | co_organizer)
        └── orgs
              └── events
                    ├── registrations  (status: not_approved → admin_approved → approved)
                    │     └── checkins
